...
import com.example.utilstest.TestService;
...
//        csvPrinter = CSVFormat.DEFAULT.withHeader("姓名", "年龄", "家乡").print(osw);
    ...
    @Test
    public void testExportQuatation() {
        String filePath = "file/export/a.csv";
        List<String> rowData = new ArrayList<>(10000);
        String header = "name,gender,num";
        rowData.add(header);
        String row = "chengdu,male,\"123,456,789\"";
        for (int i = 0; i < 100; i++) {
            rowData.add(row);
        }
        ExportFileUtil.exportFile(filePath, rowData, StandardCharsets.UTF_8, true);
    ...
    @Test
    public void testCommonCsvParse() {
        try (
        InputStream is = new FileInputStream("file/export/a.csv");
        InputStreamReader isr = new InputStreamReader(is, "GBK");
        Reader reader = new BufferedReader(isr)) {
            CSVParser parser = CSVFormat.EXCEL.withHeader("name","gender","num").parse(reader);
            Iterator<CSVRecord>  recordIterator = parser.iterator();
            while (recordIterator.hasNext()) {
                CSVRecord csvRecord = recordIterator.next();
                System.out.println(csvRecord.get("num"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
